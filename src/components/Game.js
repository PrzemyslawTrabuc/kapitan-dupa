...
class Game extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      currentGameWindow: "Menu",
      mainGameImage: spoczynek_png,
      Score: 0,
      currentKwiatyLotosu: 0,
      isPressed: false,
      isGameActive: true,
    };
    this.kwiatyLotosu = [
  ...
  StartGame = () => {
    ...
    let gameDuration = 30;
    const myInterval = gameDuration / 6;
    const Timer = () => {
      gameDuration = gameDuration - myInterval;
      // eslint-disable-next-line default-case
      switch (true) {
        case gameDuration >= 5 * myInterval:
          this.setState({ currentKwiatyLotosu: 1 });
          break;
        case gameDuration >= 3.99 * myInterval:
          this.setState({ currentKwiatyLotosu: 2 });
          break;
        case gameDuration >= 3 * myInterval:
          this.setState({ currentKwiatyLotosu: 3 });
          break;
        case gameDuration >= 1.99 * myInterval:
          this.setState({ currentKwiatyLotosu: 4 });
          break;
        case gameDuration >= 0.99 * myInterval:
          this.setState({ currentKwiatyLotosu: 5 });
          break;
        case gameDuration <= myInterval:
          clearInterval(interv);
          this.setState({ currentKwiatyLotosu: 6, isGameActive: false });
          setTimeout(
            () => this.setState({ currentGameWindow: "GameOver" }),
            3000
          );
          break;
      }
  ...
  render() {
    if (
      this.state.currentGameWindow === "Game" &&
      this.state.isGameActive === true
    ) {
      return (
        <div id={"main_game_window"}>
          ...
          <GameBar
            ...
            currentKwiatyLotosu={
              this.kwiatyLotosu[this.state.currentKwiatyLotosu]
            }
      ...
      return (
        <div id={"main_game_window"}>
          ...
          <img
            alt="głowny ekran gry"
            className="darken"
            src={this.state.mainGameImage}
          />
          <GameBar
            handleClickFireButton={this.handleClickFireButton}
            Score={this.state.Score}
      ...
      this.state.currentGameWindow === "GameOver" &&
      this.state.isGameActive === false
    ) {
      return (
        <div id={"main_game_window"}>
          <img
            alt="główny ekran gry"
            src={this.state.mainGameImage}
            style={{ opacity: 0 }}
          />
      ...
      return (
        <MainMenu
          changeGameWindow={this.changeGameWindow}
